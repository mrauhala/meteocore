...
1. [#777](https://github.com/mrauhala/meteocore/issues/777): refine coalesced-miss workspace admission and sequential encoded/codec-buffer lifetimes. Common cold layouts now prepay encoded/intermediate/scratch headroom; unknown or oversized estimates use serial actual-size admission. More precise size estimates and additional layouts can improve concurrency. Compressor-private contexts, other codecs, and coordinate/metadata discovery remain outside these explicit bounds.
