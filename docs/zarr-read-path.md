...
1. Bound remaining codec allocations (including Blosc output/private scratch) and refine native workspace admission for cache hits, encoded/intermediate headroom, and sequential-buffer lifetimes. Encoded-object admission and gzip/zstd output bounds are implemented; they do not enforce limits on other codecs or coordinate/metadata discovery.
