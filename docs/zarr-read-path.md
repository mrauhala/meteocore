...
One decode workspace adds `4 × (largest touched native decode unit + its shard index)`. Chunk shapes include stored padding and all forecast leads, even when the selected subset is tiny. Exclusively sharded partial reads use inner chunks; full-shard fast paths and shards with outer transforms use outer chunks. The Icechunk reader splits whole windows into inner chunks, avoiding that full-shard fast path even with decoded retention disabled. Admission adds one workspace per concurrent chunk, up to four touched inner chunks. It selects the largest fan-out that fits currently available memory, down to one, atomically with the reservation. The permit carries this limit to the reader. Plain-Zarr and outer-transform reads reserve one workspace.
...
This is an admission estimate, not an allocator-enforced RSS limit. Codec-private scratch, transport buffering, Icechunk internal overfetch/copying beyond the requested ranges, persistent catalog/coordinate metadata, resident caches, and API output buffers remain outside it. Native decode-workspace estimates still apply to cache hits, so low limits can reject reads whose actual working set would be smaller. Encoded sizes are discovered after native admission; tight capacity can reject a read even when smaller fan-out or earlier release of sequential encoded buffers could have fit. These refinements remain follow-up work. `zarr_read_reserved_bytes`, `zarr_read_capacity_bytes`, and `zarr_read_rejected_total`, with matching Grafana panels, expose admitted estimates and rejections.
...
1. Enforce codec-specific scratch/output limits and refine native workspace admission for cache hits, encoded headroom, and sequential-buffer lifetimes. Encoded-object admission is implemented, but the pinned zarrs gzip/zstd decoders expose no memory-limit option and can allocate independently of the declared decoded representation. This requires a codec-layer follow-up; increasing an estimate alone would not enforce a limit.
