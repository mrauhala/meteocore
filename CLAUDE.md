...
- **Icechunk (feature `icechunk`, #335):** a `[collections.zarr.icechunk]` table
  ...
  Zarr (e.g. the dynamical.org AIFS/GFS/ICON-EU datasets). Off by default — a
  heavy dep tree (`icechunk` + `zarrs_icechunk` + `zarrs/async`); the engine
  errors clearly if the table is set without the feature built. Repo location
  reuses `data_path`/`endpoint`+`bucket`+`path`; the table picks the version
  (`branch` HEAD default `main`, or `tag`, or `snapshot`). Implementation:
  ...
  tests; safe because retrieval is `concurrent_target(1)`). Icechunk owns its
  own object storage, so this path does **not** use `ds-storage`. New snapshots
  on a branch are picked up on **reload**, not poll (v1). Network-free e2e test
  generates a local repo: `cargo test -p engine-zarr --features icechunk`.
