...
- **Decode to compact native types, not `Vec<Option<f64>>`.** Boxing every sample to `Option<f64>` is a 16× memory blowup and allocator churn; carry native ints with a nodata sentinel (#206). The mechanism: `RasterTile.values` is the **`RasterValues` enum** (`F64` boxed universal form | `U8 { data, nodata, gain, offset }` raw bytes) — integer render paths should produce `U8`, which `ds-render` colorizes through a 256-entry LUT indexed by the raw byte (built per call from the style's colormap; entry i ≡ `colormap.color(value_at(i))`, so the variants are pixel-identical by construction — pinned by `u8_lut_colorize_matches_boxed_f64_exactly`). GeoTIFF's map-render path produces `U8` for local u8 sources with an integer u8 nodata (`reader::read_bbox_u8`, self-gating with an `Ok(None)` → boxed fallback); every other engine constructs `F64` via `.into()`. Adding a variant = exhaustive-match compile errors in `colorize`/`value_at` — keep them consistent.
