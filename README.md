...
| `MC_ZARR_READ_MEMORY_MB` | `1024` | Process-wide Zarr/Icechunk source-read admission budget in MiB, additional to output and cache budgets. Reserves native windows, conversion buffers, and a decode-workspace estimate before payload reads; adds encoded-buffer allowances before collection. Native reservations last through sampling, encoded reservations through retrieval. Oversized or competing reads fail immediately with 503; `0` rejects on-grid data reads. Shared across collections and reloads; restart to change. |
...
- **Read memory:** `MC_ZARR_READ_MEMORY_MB` limits admitted source working sets across Maps/WMS/Tiles and EDR, even for a tiny output image spanning a large native grid. The estimate includes native/typed/f64 buffers, window axes, and four native decode/index buffers, using full stored chunk shapes (including edge padding and forecast leads). Native reservations survive until sampling finishes. Encoded full-object/range reads add a two-copy allowance before collection and retain it through retrieval, including compressed-cache reads. Numeric-variable gzip/zstd outputs are capped by their declared codec representation; bounded intermediate outputs acquire additional capacity allowances before allocation. Other codecs (including Blosc), codec-private scratch, transport/internal storage buffers, persistent catalog metadata, resident caches, and API output buffers remain outside the estimate; this is not a total process-RAM limit. See [read-path details](docs/zarr-read-path.md).
...
- Icechunk reads up to four inner chunks concurrently through a shared four-worker pool, including when decoded retention is disabled. Source-memory admission reserves each active workspace and reduces concurrency when memory is tight. Single-chunk reads stay on the caller; plain Zarr and shards with outer transforms retain serial retrieval. Workers preserve request deadlines and finish before releasing reservations. Storage operations honor request deadlines; background operations have a 30-second timeout.
