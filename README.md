...
| `MC_ZARR_READ_MEMORY_MB` | `1024` | Process-wide Zarr/Icechunk source-read admission budget in MiB, additional to output and cache budgets. Reserves native windows, conversion buffers, and a decode-workspace estimate before payload reads; holds the reservation through sampling. Oversized or competing reads fail immediately with 503; `0` rejects on-grid data reads. Shared across collections and reloads; restart to change. |
