...
<main id="main" tabindex="-1">{body}</main><footer class="page-footer"><span>MeteoCore · Open weather data</span><span>HTML representation · Times in UTC</span></footer></div></div><dialog id="help-dialog"><div class="dialog-head"><h2>Find the right data</h2><button class="icon-button" id="close-help" aria-label="Close help">×</button></div><div class="panel-body"><p>Collection search matches titles, descriptions and keywords. Commas separate alternatives; words form a phrase.</p><p>Advanced search adds required (+) and excluded (-) terms. Area and time narrow coverage; unknown extents remain eligible.</p><p>Item property filters belong to the selected collection. The JSON switch always opens the current resource with its applied filters and paging.</p></div></dialog><div id="toast" role="status" aria-live="polite"></div><script>{SCRIPT}</script></body></html>"##,
            title = escape(title),
...
pub fn landing_document(
    ...
    let example = format!("{base}/{discovery}/collections?q=radar&limit=12&f=html");
    body.push_str(&format!("<div class=\"developer-start\"><section class=\"panel panel-body\"><span class=\"eyebrow\">BUILD A DISCOVERY REQUEST</span><h2>Find a collection</h2><form method=\"get\" action=\"{base}/{discovery}/collections\"><input type=\"hidden\" name=\"f\" value=\"html\"><label for=\"q\">q <span class=\"parameter-type\">string · optional</span></label><div class=\"search-row\"><input id=\"q\" name=\"q\" placeholder=\"radar\"><button class=\"btn primary\">Build query {arrow}</button></div><p class=\"field-help\">Matches title, description and keywords. Add spatial, temporal and paging parameters in the query builder.</p></form></section><section class=\"panel panel-body\"><span class=\"eyebrow\">REQUEST EXAMPLE</span><code class=\"example-code\">GET /{discovery}/collections?q=radar&amp;limit=12&amp;f=json</code><p class=\"section-note\">HTML and JSON represent the same resource and applied parameters.</p>{}</section></div>",anchor(&example,"Open in query builder →","btn small"),base=escape(base),arrow=icon("arrow")));
...
pub fn collection_html(
    ...
    if let Some(items) = items {
        body.push_str(&anchor(&with_format(items, "html"), "Browse items", ""));
    }
    ...
    body.push_str(&format!("<div class=\"coverage-facts\"><div class=\"fact\"><small>Start · UTC</small><strong>{}</strong></div><div class=\"fact\"><small>End · UTC</small><strong>{}</strong></div><div class=\"fact wide\"><small>Advertised bounds · west, south, east, north</small><strong class=\"mono\">{}</strong></div></div></section><section class=\"section-space\"><h2>What you can do with this collection</h2>",value_html(&interval[0]),value_html(&interval[1]),value_html(bbox)));
    if let Some(items) = items {
        body.push_str(&format!("<div class=\"callout spaced\">Browse individual items, filter by supported properties, inspect geometry, and access the same results as GeoJSON.</div><div class=\"action-row spaced\">{}{}</div>",anchor(&with_format(items,"html"),"Browse items →","btn primary"),anchor(&url,"View metadata JSON","btn")));
    }
...
pub fn collections_html(
    ...
    for parameter in CollectionParameter::ALL {
        ...
        match parameter {
            ...
            CollectionParameter::Q=>primary=format!("<div class=\"search-row\"><div class=\"search-main\"><label for=\"collection-search\">q <span class=\"parameter-type\">string · optional</span></label><div class=\"search-input-wrap\">{}<input id=\"collection-search\" name=\"q\" value=\"{}\" placeholder=\"radar\"></div></div><button class=\"btn primary\">Apply query {}</button></div><p class=\"search-hint\">Titles, descriptions and keywords.<span>Try {} or {}</span></p>",icon("search"),escape(current),icon("arrow"),anchor(&query_edit(&json_url,"q",Some("radar")),"radar","quiet"),anchor(&query_edit(&json_url,"q",Some("observations")),"observations","quiet")),
    ...
    let chips = pairs
        ...
        .collect::<String>();
    let form=format!("<form class=\"search-panel query-form\" method=\"get\" action=\"{}\"><input type=\"hidden\" name=\"f\" value=\"html\"><div class=\"builder-heading\"><h2>Query parameters</h2><span class=\"mono\">GET</span></div>{primary}<details class=\"filters\" open><summary>Advanced search, area and time</summary><div class=\"filters-grid\">{advanced}</div><div class=\"paging-fields\">{paging}</div><div class=\"filter-controls\"><span class=\"field-help\">Collections with unknown extents remain eligible.</span>{}</div></details><div class=\"active-filters\">{chips}</div><div class=\"draft-request enhanced\"><small>Request preview · apply to update results</small><code data-draft></code></div><noscript><p>Enable JavaScript to edit advanced parameters.</p></noscript></form>",escape(url),anchor(&with_format(url,"html"),"Clear filters","quiet"));
    let mut body = page_heading(
        "Collections",
        "GET /collections · Build a discovery query and inspect the matching resources.",
    );
